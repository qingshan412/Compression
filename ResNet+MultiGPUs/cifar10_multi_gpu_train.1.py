...
tf.app.flags.DEFINE_string('train_dir', './tmp/cifar10_train_multi',
...
def tower_loss(scope):
  ...
  with tf.variable_scope(tf.get_variable_scope()):
    ...
    images, labels = cifar10_input.build_input(dataset='cifar10', data_path = FLAGS.data_dir, 
                                                    batch_size=FLAGS.batch_size, mode='train')

  
    # Build inference Graph.
    logits = cifar10_net.inference_resnet_20(images)

    # Build the portion of the Graph calculating the losses. Note that we will
    # assemble the total_loss using a custom function below.
    _ = cifar10_net.loss(logits, labels)
...
def average_gradients(tower_grads):
  ...
  for grad_and_vars in zip(*tower_grads):
    ...
    #eprint(grads)
    #exit(0)
    #grad = tf.concat(tf.to_float(0), grads)
...
def eval_in_train():
  with tf.variable_scope(tf.get_variable_scope()):
    # Eval
    images_e, labels_e = cifar10_input.build_input(dataset='cifar10', data_path = FLAGS.data_dir, 
                                                    batch_size=FLAGS.batch_size, mode='test')
    ...
    for step in xrange(FLAGS.max_steps):
      ...
      if step % 100 == 0:
        ...
        format_str = ('%s: step %d, loss = %.2f, accuracy = %.2f (%.1f examples/sec; %.3f '
                      'sec/batch)')
        print (format_str % (datetime.now(), step, loss_value, accur_value, 
                             examples_per_sec, sec_per_batch))
...
def main(argv=None):  # pylint: disable=unused-argument
  #cifar10_net.maybe_download_and_extract()
